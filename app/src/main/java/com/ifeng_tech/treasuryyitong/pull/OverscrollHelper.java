			...
			if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {
				...
				if (newScrollValue < (0 - fuzzyThreshold)) {
					...
					if (mode.showFooterLoadingLayout()) {
						...
						// overscrolling, so change the state
						if (currentScrollValue == 0) {
							view.setState(com.ifeng_tech.treasuryyitong.pull.PullToRefreshBase.State.OVERSCROLLING);
						}
					...
					view.setState(com.ifeng_tech.treasuryyitong.pull.PullToRefreshBase.State.RESET);
				}
			} else if (isTouchEvent && com.ifeng_tech.treasuryyitong.pull.PullToRefreshBase.State.OVERSCROLLING == view.getState()) {
