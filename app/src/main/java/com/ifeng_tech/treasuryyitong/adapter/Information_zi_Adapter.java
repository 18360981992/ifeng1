...
public class Information_zi_Adapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if(getItemViewType(position)==0){
            ...
            if(topList.get(position).getPictureUrl()==null){
                information_zi_fragment_buju_right_img.setImageResource(R.drawable.img_erroy);
            }else{
                String pictureUrl = topList.get(position).getPictureUrl();
                try {
                    JSONObject jsonObject = new JSONObject(pictureUrl);
                    String appUrl = (String) jsonObject.get("appUrl");
                    Glide.with(context).load(appUrl).error(R.drawable.img_erroy).into(information_zi_fragment_buju_right_img);
            ...
            if(topList.get(position).getAppUrl()==null){
                ...
                Glide.with(context).load(topList.get(position).getAppUrl()).error(R.drawable.img_erroy).into(information_zi_fragment_buju_bottom_img1);
                Glide.with(context).load(topList.get(position).getAppUrl()).error(R.drawable.img_erroy).into(information_zi_fragment_buju_bottom_img2);
                Glide.with(context).load(topList.get(position).getAppUrl()).error(R.drawable.img_erroy).into(information_zi_fragment_buju_bottom_img3);
            }
