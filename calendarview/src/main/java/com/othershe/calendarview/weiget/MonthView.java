...
public class MonthView extends ViewGroup {
    ...
    public void setSevendate(List<String> sevendate){
...
//                }
                ...
                String i1 = String.valueOf(date.getSolar()[0]);
                String i2 = String.valueOf(date.getSolar()[1]);
                String i3 = String.valueOf(date.getSolar()[2]);
                if(i2.length()==1){
                    i2="0"+i2;
                }
                if(i3.length()==1){
                ...
                String ss = i1+"-"+i2+"-"+i3;
                boolean contains = sevendate.contains(ss);
                if(contains){
                    solarDay.setTextColor(mAttrsBean.getColorSolar());
                }
            }else{
                // 设置已经过去时间的阳历颜色
                if(date.getSolar()[1]>CalendarUtil.getCurrentDate()[1]){
                    solarDay.setTextColor(mAttrsBean.getColorSolar());
                }else if(date.getSolar()[1]<CalendarUtil.getCurrentDate()[1]){
                    solarDay.setTextColor(mAttrsBean.getColorLunar());
                }else{
                    if(date.getSolar()[2]<CalendarUtil.getCurrentDate()[2]){
                        solarDay.setTextColor(mAttrsBean.getColorLunar());
                    }else{
                        solarDay.setTextColor(mAttrsBean.getColorSolar());
                    }
                ...
                if ("初一".equals(date.getLunar()[1])) {
                    ...
                    if (!TextUtils.isEmpty(date.getSolarHoliday()) && mAttrsBean.isShowHoliday()) {//阳历节日
                        ...
                        if (TextUtils.isEmpty(date.getLunar()[1])) {
                            lunarDay.setVisibility(INVISIBLE);
                        } else {
